...
\jcom{I would include a precise citation for this. Also, this is a nice general remark for the fourier transform, but aren't we implicitly
assuming regularity in the $L_2$ sense?}
...
From the graph we can say that beyond $\omega=15 $, H becomes of negligible values, thus we can set
\begin{center}
$\omega_{max}=(N-\frac{1}{2})\Delta \omega=15$\\
$\Delta \omega= \frac{\omega_{max}}{N-\frac{1}{2}}$

\end{center} 
\jcom{Like I explained earlier, one may not choose the cutoff in this way. Come back to the counterexample I gave you on a whiteboard where the characteristic function is asymptotically $\bigo{\absval{\omega}^{-1}}$. If you want to do the cutoff using a fixed cutoff point that assures that the truncation error has at most the magnitude of the floating point errors, you need to give justification for it and possibly explain the tradoffs that come with that choice, namely poorer convergence.}
...
\jcom{Do you have a plot that shows how your different methods converge to the same result?}
...
To evaluate the DFT we need to identify and quantify the total erro $\epsilon$ of the method.
 $\epsilon$ can be splited into a sum of two terms: the quadrature and truncation errors. The former is the error from the approximation of the integral in \eqref{ghat} by an infnite sum, while the latter is due to the truncation of the infnite sum to a sum over N points .\\
...
The error of an N-point approximation converges at an incredible $\mathbb{O}$ ($-\frac{c}{N^2}$ )rate
\jcom{Please produce a plot and put your stated line as a reference line. Also please include the derivation of what you assume for
this stated result to hold. My understanding is that this is true for non-hardy functions whose second derivative is boudned on closed intervals.}
