...
\end{equation}
By setting $h_k=  e^{-i k x_{min} \Delta \omega } \hat{f}_{\alpha}(\omega_k,\tau)$ and $(\omega_n^{j k}= e^{-i \frac{2\pi}{N}j k })_{0 \le j<N , 0 \le k<N}$ we can apply the FFT approach and the option price \jcom{????}.
\begin{equation}
...
The Fourier transform method is known to be very efficient with a variety of models especially Levy processes in spite of having no closed form expressions for their respective probability density functions.\\

We have seen that option price can be obtained via samples of the characteristic function and the Fourier Transform of the payoff.Instead of applying an integral that involves the inner product of the terminal payoff and the density function of the Levy process, it will be easier to compute the integral of their Fourier transform as the characteristic function  of the Levy process is easier to be handled than the density function itself.\\
...
In this chapter we will introduce a method allowing us to obtain an option price under the following conditions: we have an analytical form expression of the payoff and the characteristic functions but we do not know a closed form expression of the Fourier transform of the payoff and the probability density functions.Thus we are not able to compute the expectation integral neither in the real space nor in the Fourier space .\\ 

\begin{center}
 
$g(x)$ ~$\rightarrow$  ~  $ \hat g (\omega) $\\
${p_T}(x)$ ~  $\leftarrow$ ~  $ \varphi (\omega)$
\end{center}

The aim of this chapter is to develop a mathematical formulation followed by a numerical convergence approximation to prove that using an FFT technique applied to the payoff and the characteristic functions and using Parseval equality we can work in the Fourier space or the real space , they  will exactly gives the same result.()
