...
\section{Theoretical Framework and Method Used}
\begin{gather}
\pi(\tau,x)= e^{-r\tau} \expp{G \parent{X_T}|X_{T-\tau} = x}
\label{ff}
\end{gather}
...
\end{center}
\section{ Parseval equality in the discrete space } 
\begin{center}
$\sum \limits_{n=0}^{N-1} x(n)y^*(n)= \frac{1}{N} \sum \limits_{k=0}^{N-1} X(k)Y^*(k)$
\end{center} 


we wil start the proof by using the FFT approach to extract a vector the distribution function from the characteristic function :
