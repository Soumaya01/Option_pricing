...
\jcom{I would try to avoid using $\epsilon$, $o$ and $O$ as variables}
...
\end{gather*}
\jcom{Can you give a derivation or justification for this? How is this related to the conditioning with respect to jump sizes that you speak about earlier?}
...
\jcom{Do you have some numerics that you can present that show that the se two prices
coincide with the FFT results?}
...
\jcom{Why would you apply FFT to both????}
...
We will prove that without knowing the expression of $\hat g (\omega)$ and $p_T(x)$ we still have a good approximation for the option price in eq \eqref{ff}.
...
\end{center}
\textbf{Parseval equality in the discrete space :} 
\begin{center}
\label{sum}
$\sum \limits_{n=0}^{N-1} x(n)y^*(n)= \frac{1}{N} \sum \limits_{k=0}^{N-1} X(k)Y^*(k)$
\end{center} 
we wil start the proof by using the FFT approach to extract a vector of the distribution function from the characteristic function.
...
\end{align*}
Thus by applying the same steps as in eq \eqref{4.14} we will get :
\begin{align*}
p_T(x_j)=&\frac{1}{2 \pi} e^{- w_{min} x_j}\sum_{k=0}^{N-1} e^{- i k \frac{2 \pi}{N}} e^{-i \Delta\omega x_{min} } \varphi(\omega_k) \\
=&\frac{1}{2 \pi} e^{- iw_{min} x_j} FFT\big ( e^{-i \Delta\omega x_{min} } \varphi(\omega_k) \big) ~~for~ 0 \leqslant j < N\\
\end{align*}
The same approach is followed to obtain ${\hat g (\omega_k) } from ~g(x_j)$:
\begin{align*}
\hat g (\omega_k)=& e^{i w x_{min}} iFFT\big ( e^{i \Delta x w_{min} } g(\omega_j) \big) ~~for~ 0 \leqslant k < N\\
\end{align*}
\jcom{Use italics only for the variables, not for operators, if possible}
...
\jcom{I understand that you claim to have proven the quality of the right hand sides of equations \eqref{rm1} and \eqref{rm2}.
However, I don't see how this is proven. Furthermore, the furthest right hand side of \eqref{rm1} does not depend on the payoff function,
...
\section{Second framework: integration by parts }
