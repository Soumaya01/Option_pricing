...
\textcolor{blue}{i am not seeing any variable with these name} 
...
Kou and wang suggested the following approach in order to compute a closed form solution of an option price.~First of all we need to know the sum of double exponentially random variable.
 ...
 \[\theta= \begin{cases} 
  1~ if \beta >0 ~and ~\alpha ~\neq  0\\
    -1~ if \beta < 0 ~and ~\alpha ~\leqslant 0\\
         \end{cases}
          \]
The option price will be defined as \jcom{Is this the definition? What option price, where is the strike and how is the $T$ connected to $t$ and $\tau$?},
...
\jcom{Good. Please include the code and all the documentation therein, as well as a plot
that demonstrates that your FFT results are the same as the price obtained using \eqref{rmeukou}.
This way you will demonstrate that you have two independent implementations of the code, and they
both work.}
