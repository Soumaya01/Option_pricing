...
pixel rasterisations, when possible.}
...
\jcom{I don't have a bibliography at all here...}
...
\\
= &\int_\R g\parent{y}  p_\tau \parent{y-x}  \d y
\\
\hat \fa \parent{\tau,\omega}= &  \int_\R \int_\R \expfs{- \damp x + i \omega x} g \parent{y}  p \parent{y-x} \d y \d \omega
\\
...
Assume we have a Merton Jump-Diffusion with parameters , ,  and  and r is the risk-free interest
rate,  the time to maturity, S the spot price and K the strike price. If we setoption price given by the Merton Jump-Diffusion is given by
\begin{figure}[h]

\centering
\includegraphics[scale=0.7]{Iteration_Max.jpg} 
\caption{Compound Poisson process with intensity $\lambda=8$}
\end{figure}
