...
\item $\lim\limits_{\omega \rightarrow +\infty} sup _{} f(x)$

\end{itemize}

Then consider $M()=\int_a^b \! f(x) \, \mathrm{d}x$.





Theorem 6.1 in Trefethen and Weideman (2013) states that :
 Suppose $\omega$ is analytic in the strip $|Im [x]| < a$ for some a > 0. Suppose further that $w(x) \rightarrow 0$ uniformly as x in the strip, and for some M it satisfies:
\begin{center}
$\int $
\end{center}
