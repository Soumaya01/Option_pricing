...
In total, we need $O(N^2)$ operations to compute directly the DFT. The FFT reduces the complexity of the previous algorithm from order  $O(N^2)$ to order $O(N log_2N)$.\\
 Although it appears to  have the same computation time,we will figure out in the table below that The fast Fourier transform  reduce significantly the number of operation to perform and it makes a big difference when N is very large.
\begin{table}[h!]
\begin{center}
 \begin{tabular}{||c c c ||} 
 \hline
...
\end{table}
 ...
 We can clearly notice that the summation in \eqref{fret} is not a direct application of the FFT algorithm. We need to modify it in a way 
