...
The Lévy measure of the VG model is given by:
 ...
 \[\nu_{VG}(y)= \begin{cases} 
  \frac{K e^{y \eta_+}}{y}~ if ~y >0 \\
    
    \frac{K e^{y \eta_-}}{y}~ if y \leq 0 
         \end{cases}
          \]
...
We consider a Lévy process $B(t,\theta,\sigma)$ defined in terms of a Brownian motion with drift $\theta$ and volatility $\sigma$, and a gamma process with unit mean rate $\gamma(t,1,\mu)$
\jcom{So how is this gamma process related to $B(t,\theta,\sigma)$ ?}
