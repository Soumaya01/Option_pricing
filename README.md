...
By a Discrete Fourier Transform we can obtain an option pice approximation (DFT code).In some cases (the characteristic function and the fourier transform of the payoff are known analytically, the characteristic function is known but the fourier transform of the payoff is unknown and when calculating the Fourier transform of the payoff is relatively difficult), we can apply FFT algorithm to value options both in the real and fourier spaces (price FFT_real,price FFT_fourier).
